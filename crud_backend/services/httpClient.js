...
function post(url, params = {}, headersJson = {}) {
    ...
    restClient.post(url, {
        ...
        if ((!data || (response.statusCode !== statusCodes.OK && response.statusCode !== statusCodes.CREATED))) {
            if (Buffer.isBuffer(data)) {
                data = data.toString();
            }
            console.error('Error in POST url:', url, params, response.statusCode, data);
            deferred.resolve({
                result: null,
            ...
            deferred.resolve({ result: createJSONResp(response, data), headers: response.headers, statusCode: response.statusCode });
        }
    });
